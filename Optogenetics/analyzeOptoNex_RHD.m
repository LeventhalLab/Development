function [pulse_binary,pulse_ts] = analyzeOptoNex_RHD(ephysData,laserData,nexFile,Fs,dosteps,protocol,useUnits,power)

% % dosteps = [1,2];
% % protocol = 1; % 1 = long, 2 = 50hz, 3 = 100hz, 4 = 20hz
% % useUnits = [2];


if protocol == 1
    pethWindow = 10; % seconds
    pethBinWidth = 250; % ms
    minResetTime = 1; % seconds
    pulseBreaks = 501:501:2505;
    f='0Hz';
    
elseif protocol == 2
    protocolFreq = 20;
    pethWindow = .05;
    pethBinWidth = 1.25; %pethBinWidth = pethWindow/10/4*1000;
    minResetTime = 0;
    pulseBreaks = 201:201:1005; %pulseBreaks = protocolFreq*10+1:protocolFreq*10+1:((protocol*10+1)*5);
    f=[num2str(protocolFreq) 'Hz'];
...
%         [v,k] = sort(sortArr);
        ...
        xlabel('time (s)');
        ylabel('pulse');
        title({fileName,['Unit ' num2str(iUnit)] ,[num2str(numel(pulse_ts)),' pulses, ' power ' ' f]},'interpreter','none');
        hold on;
