...
for iNeuron = 1%:366
    ...
    curTs_random = rand([1 nSamples*10]) * max(curTs);
...
%     W = squeeze(W);
    ...
    for ii = 1:2
        ...
        for iSpike = 1:numel(useTs)
            if useTs(iSpike) < tWindow || useTs(iSpike) > tsEnd - tWindow
                continue;
            end
            Wcenter = find(tW > useTs(iSpike),1);
            Wrange = Wcenter - Wsamples:sampleInt:Wcenter + Wsamples;
            powerData = abs(W(Wrange,:)).^2;
            phaseData = angle(W(Wrange,:));
            if max(max(powerData)) < 1e5
                spikeCount = spikeCount + 1;
                STAArr_power(ii,spikeCount,:,:) = powerData;
                STAArr_phase(ii,spikeCount,:,:) = phaseData;
            else
                disp('overpower');
            end
            if spikeCount >= nSamples
                break;
            end
        end
    end
    
    STAArr_power_surr = squeeze(mean(squeeze(STAArr_power(2,:,:,:))));
    STAArr_power_zMean = mean(STAArr_power_surr);
    STAArr_power_zStd = std(STAArr_power_surr);
    STAArr_power_shuf = squeeze(mean(squeeze(STAArr_power(1,:,:,:))));
    STAArr_power_z = (STAArr_power_shuf - STAArr_power_zMean) ./ STAArr_power_zStd;
    
    STAArr_phase_surr = squeeze(circ_r(squeeze(STAArr_phase(2,:,:,:))));
    STAArr_phase_zMean = mean(STAArr_phase_surr);
    STAArr_phase_zStd = std(STAArr_phase_surr);
    STAArr_phase_shuf = squeeze(circ_r(squeeze(STAArr_phase(1,:,:,:))));
    STAArr_phase_z = (STAArr_phase_shuf - STAArr_phase_zMean) ./ STAArr_phase_zStd;
    
    h = figuree(250,500);
    t = linspace(-tWindow,tWindow,size(STAArr_power_z,1));
    subplot(211);
    imagesc(t,freqList,STAArr_power_z');
    title({['u',num2str(iNeuron,'%03d')],'Power'}); hold on;
    
    subplot(212);
    imagesc(t,freqList,STAArr_phase_z');
    title('MRL'); hold on;
    ...
    for iSubplot = 1:2
        ...
        xticks(sort([0 xlim]));
        curyLim = ylim;
        ytickLocs = linspace(curyLim(1),curyLim(2),size(STAArr_power_z,2));
        yticks(ytickLocs);
