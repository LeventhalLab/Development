...
for iTiming = 1%:2
    for iPower = 1:2
        ...
        for iEvent = 1:numel(useEvents)
            ...
            data = squeeze(mean(use_rhos{iPower}(:,iTiming,useEvents(iEvent),:,:)));
            ...
            data = squeeze(use_pvals{iPower}(:,iTiming,useEvents(iEvent),:,lookatFreqs(iPower)));
            ...
            if doLabels
                ylabel('Freq (Hz)');
                cbAside(gca,'r','k');
                title({eventFieldnames{useEvents(iEvent)},[timingFields{iTiming},' - ',powerLabel{iPower}]});
...
if doSave
    setFig('','',[1.5,2.75]);
    print(gcf,'-painters','-depsc',fullfile(figPath,'RTMTCORR_sessionLines.eps'));
