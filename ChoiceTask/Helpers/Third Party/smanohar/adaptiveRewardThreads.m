...
if length(threads) >= thread  % already initialised: get parameters
  h = threads(thread); 
elseif length(threads)>0 % if one thread is initialised, copy its parameters
  h = struct();
  h.historyLength = threads(1).historyLength;
  h.increment     = threads(1).increment;
  h.minPercentile = threads(1).minPercentile;
  h.maxPercentile = threads(1).maxPercentile;
  h.tRecent       = [];
  h.tMin          = threads(1).tMin;
  h.tMax          = threads(1).tMax;
...
if strcmpi(t,'GetParameters')
  reward=[h.historyLength h.increment h.maxPercentile h.minPercentile h.tMin h.tMax];
  return;
end;
