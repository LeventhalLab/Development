...
for ii=1:15
    ...
    for jj=1:eventLength
        ...
        subplot(4,1,1);
        hold on;
        plot(eventTs,fdata_33(round(ts(archive_burst_start(ii)+jj)*header.Fs)),'o','color','black');
        
        subplot(4,1,2);
        hold on;
        plot(eventTs,fdata_35(round(ts(archive_burst_start(ii)+jj)*header.Fs)),'o','color','black');
        
        subplot(4,1,3);
        hold on;
        plot(eventTs,fdata_37(round(ts(archive_burst_start(ii)+jj)*header.Fs)),'o','color','black');
        
        subplot(4,1,4);
        hold on;
        p(2) = plot(eventTs,fdata_39(round(ts(archive_burst_start(ii)+jj)*header.Fs)),'o','color','black');
    end
    
    for jj=1:length(ts_bEvents)
        eventTs = ts_bEvents(jj)-ts(archive_burst_start(ii));
    ...
    for jj=1:length(ts_cEvents)
        eventTs = ts_cEvents(jj)-ts(archive_burst_start(ii));
