function fredCombineNex(sessionConf)
%Fred's combinedNex version, with bits and pieces from past version
%nSampleRate = sampling Rate of system
%sessionName = session
%First dialogue box = select all nex files you want to combine
%Second dialogue box allows you to select where to save the combined nex
%Writes the neuron names as the last 4 characters of the original nex file
%name, i.e. 'W01' or 'T01'
...
%                 newUnitName = [sessionName '_' unitID];
                ...
                fileName = nexfn{iNex}
                newName = fileName(length(fileName)-21:length(fileName)-19)
                newUnitName = [sessionConf.sessionName,'_',newName,'_',num2str(iVar)]; 
